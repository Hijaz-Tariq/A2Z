  ...
  useEffect(() => {
    const getParcel = async () => {
      try {
        ...
        const res = await publicRequest.get("/users/find/" + senderemail);
        setUser(res.data);
      ...
      {Array.from({ length: total }).map((_, index) => (
        <div key={index} id="invoice" className="p-3">
          <div className="bg-white ">
            ...
            <h1 className=" ">{parcel.recipientname}</h1>
            <h1 className=" ">{parcel.recipientphone}</h1>
            <h1 className="">{parcel.to}</h1>
            ...
            <div className="flex justify-between pt-4 mt-4">
              <h3 className="flex flex-col">{user.fullname}</h3>
              <footer></footer>
            </div>
            <div className="flex justify-center items-center">
  <h1 className="pt-2 mt-2 ">www.a2z-express.com</h1>
</div>
          </div>
        </div>
      ))}
